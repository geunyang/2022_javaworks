...
/*
 * 배열, 리스트에 담겨있는 데이터들을
 * 어떤 항목, 값을 기준으로 정렬을 할 경우가 많다
 * DBMS 와 같은 데이터 관리 도구를 사용하는 경우는
 * SQL 명령으로 쉽게 정렬을 할 수 있다
 * 하지만 배열이나 리스트에 담긴 데이터들은 
 * 그 양이 별로 많지 않기 때문에 코딩으로 데이터를 정렬할 수 있다
 * 
 * insert 정렬, bubble 정렬, selection 정렬, Quick 정렬
 * merge 정렬, shell 정렬, radix 정렬 알고리즘이 존재한다
 * 
 * 간단한 정렬 코드를 이해하면 쉽게 종렬을 수행할 수 있다
 * 
 * insert 정렬과 bubble 정렬의 중간 쯤 성능을 갖는 알고리즘이다
 * 이 알고리즘은 정식 정렬 알고리즘으로 인정 받지는 않는다
 * 
 * 전체 배열이나 리스트를 for() 문으로 반복하면서
 * 0번 위치와 1 ~ 끝 위치까지 비교하면서 큰 값은 오른쪽으로 
 * 작은 값은 왼쪽으로 
 * 1번 위치와 2 ~ 끝 위치까지 비교하면서 큰 값은 오른쪽으로
 * 작은 값은 왼쪽으로
		...
		for(int i = 0; i <nums.length; i ++) {
			for(int j = i + 1; j < nums.length; j ++) {
				...
				if(nums[i] > nums[j]) {
					...
					// 왼쪽 위치에 오른쪽 값을 저장
					// 원래의 왼쪽 값을 제거되고 오른쪽 값과 같아진다
					nums[i] = nums[j];
					
					// 임시 대피했던 값을 오른쪽 위치에 저장
					// 임시 대피했던 값은 왼쪽 값이므로
					// 결국 왼쪽 값을 오른쪽에 저장하는 것과 같다(교환)
					nums[j] = _temp;
				} // end if, 두 값 비교
			}// end for(j)
		}// end for(i)
